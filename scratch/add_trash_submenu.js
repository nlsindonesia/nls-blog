...
// 1. Kalender Submenu
const oldKalSubmenu = `                        <!-- Dropdown Submenu: Create Event & Present Event -->
...
const newKalSubmenu = `                        <!-- Dropdown Submenu: Create Event, Present Event, & Trash Event -->
                        <div x-show="isKalenderDropdownOpen" x-cloak class="border-l-2 border-sky-300 dark:border-sky-800 ml-4 pl-2.5 pt-1 space-y-1.5">
                            <!-- Submenu 1: Create Event -->
                            <button type="button" @click="openCreateEventView()"
                                :class="activeTab === 'kalender' && kalenderView === 'create' ? 'submenu-btn-active' : 'submenu-btn-inactive'"
...
content = content.replace(oldKalSubmenu, newKalSubmenu);

// 2. Berita Submenu
const oldBeritaSubmenu = `                        <!-- Dropdown Submenu: Create News & Present News -->
...
const newBeritaSubmenu = `                        <!-- Dropdown Submenu: Create News, Present News, & Trash News -->
                        <div x-show="isBeritaDropdownOpen" x-cloak class="border-l-2 border-emerald-300 dark:border-emerald-800 ml-4 pl-2.5 pt-1 space-y-1.5">
                            <!-- Submenu 1: Create News -->
                            <button type="button" @click="openCreateNewsView()"
                                :class="activeTab === 'berita' && beritaView === 'create' ? 'submenu-berita-active' : 'submenu-btn-inactive'"
...
content = content.replace(oldBeritaSubmenu, newBeritaSubmenu);

// 3. Pengajar Submenu
const oldPengajarSubmenu = `                        <!-- Dropdown Submenu: Add Teacher & Present Teacher -->
...
const newPengajarSubmenu = `                        <!-- Dropdown Submenu: Add Teacher, Present Teacher, & Trash Teacher -->
                        <div x-show="isPengajarDropdownOpen" x-cloak class="border-l-2 border-indigo-300 dark:border-indigo-800 ml-4 pl-2.5 pt-1 space-y-1.5">
                            <!-- Submenu 1: Add Teacher -->
                            <button type="button" @click="openAddTeacherView()"
                                :class="activeTab === 'pengajar' && pengajarView === 'add' ? 'submenu-pengajar-active' : 'submenu-btn-inactive'"
...
content = content.replace(oldPengajarSubmenu, newPengajarSubmenu);

// 4. Breadcrumb
const oldBreadcrumb = `x-text="activeTab === 'kalender' ? (kalenderView === 'create' ? 'Kalender Event / Create Event' : 'Kalender Event / Present Event') : (activeTab === 'berita' ? (beritaView === 'create' ? 'Berita & Artikel / Create News' : 'Berita & Artikel / Present News') : (pengajarView === 'add' ? 'Daftar Pengajar / Add Teacher' : 'Daftar Pengajar / Present Teacher'))"`;

const newBreadcrumb = `x-text="activeTab === 'kalender' ? (kalenderView === 'create' ? 'Kalender Event / Create Event' : (kalenderView === 'trash' ? 'Kalender Event / Trash Event' : 'Kalender Event / Present Event')) : (activeTab === 'berita' ? (beritaView === 'create' ? 'Berita & Artikel / Create News' : (beritaView === 'trash' ? 'Berita & Artikel / Trash News' : 'Berita & Artikel / Present News')) : (pengajarView === 'add' ? 'Daftar Pengajar / Add Teacher' : (pengajarView === 'trash' ? 'Daftar Pengajar / Trash Teacher' : 'Daftar Pengajar / Present Teacher')))"`;

content = content.replace(oldBreadcrumb, newBreadcrumb);

// 5. Kalender Trash View
const kalenderTrashView = `
                    <!-- =================================================================
...
content = content.replace('                    </div>\n\n                <!-- =========================================================================\n                     TAB 2: MANAJEMEN BERITA & ARTIKEL', '                    </div>\n' + kalenderTrashView + '\n\n                <!-- =========================================================================\n                     TAB 2: MANAJEMEN BERITA & ARTIKEL');

// 6. Berita Trash View
const beritaTrashView = `
                    <!-- =================================================================
...
content = content.replace('                    </div>\n\n                </div>\n\n                <!-- =========================================================================\n                     TAB 3: MANAJEMEN DAFTAR PENGAJAR', '                    </div>\n' + beritaTrashView + '\n\n                </div>\n\n                <!-- =========================================================================\n                     TAB 3: MANAJEMEN DAFTAR PENGAJAR');

// 7. Pengajar Trash View
const pengajarTrashView = `
                    <!-- =================================================================
...
// 8. Alpine.js datasets & methods
const oldDatasets = `                teachers: (function() {
                    try {
...
const newDatasets = `                teachers: (function() {
                    try {
                        const stored = localStorage.getItem("nls_pengajar_teachers_v1");
                        if (stored) {
                            const parsed = JSON.parse(stored);
...
content = content.replace(oldDatasets, newDatasets);

// Kalender open & delete
const oldKalOpen = `                openPresentEventView() {
...
const newKalOpen = `                openPresentEventView() {
                    this.activeTab = 'kalender';
                    this.kalenderView = 'present';
                    this.isKalenderDropdownOpen = true;
                    if (this.isMobile) this.isSidebarOpen = false;
...
content = content.replace(oldKalOpen, newKalOpen);

const oldKalDelete = `                deleteEvent(id) {
...
const newKalDelete = `                deleteEvent(id) {
...
content = content.replace(oldKalDelete, newKalDelete);

// Berita open & delete
const oldBeritaOpen = `                openPresentNewsView() {
...
const newBeritaOpen = `                openPresentNewsView() {
                    this.activeTab = 'berita';
                    this.beritaView = 'present';
                    this.articleEditor.isOpen = false; this.beritaView = 'present';
                    this.isBeritaDropdownOpen = true;
...
content = content.replace(oldBeritaOpen, newBeritaOpen);

const oldArtDelete = `                deleteArticle(id) {
...
const newArtDelete = `                deleteArticle(id) {
...
content = content.replace(oldArtDelete, newArtDelete);

// Pengajar open & delete
const oldPengajarOpen = `                openPresentTeacherView() {
...
const newPengajarOpen = `                openPresentTeacherView() {
                    this.activeTab = 'pengajar';
                    this.pengajarView = 'present';
                    this.isPengajarDropdownOpen = true;
                    if (this.isMobile) this.isSidebarOpen = false;
...
content = content.replace(oldPengajarOpen, newPengajarOpen);

const oldTeacherDelete = `                deleteTeacher(id) {
...
const newTeacherDelete = `                deleteTeacher(id) {
...
content = content.replace(oldTeacherDelete, newTeacherDelete);

fs.writeFileSync(path, content, 'utf8');
console.log("SUCCESS: Applied Trash submenus and full soft-delete & restore system to /nlsadmin!");
