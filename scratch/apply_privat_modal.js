...
let content = fs.readFileSync(privatPath, 'utf8');

// 1. Update <body ...> to include x-data="privatApp()"
content = content.replace(
    /<body\s+class="min-h-screen bg-surface font-sans selection:bg-primary-fixed selection:text-on-primary-fixed overflow-x-hidden flex flex-col text-on-surface">/,
    '<body x-data="privatApp()" class="min-h-screen bg-surface font-sans selection:bg-primary-fixed selection:text-on-primary-fixed overflow-x-hidden flex flex-col text-on-surface">'
);

// 2. Update the 3 package action buttons
const old_button_1 = `<a :href="packageWaLink(packages[0])" target="_blank" rel="noopener noreferrer"
...
const new_button_1 = `<button type="button" @click="openPackageModal('reguler')"
                                style="background: linear-gradient(135deg, #0284c7 0%, #0369a1 100%) !important; color: #ffffff !important; box-shadow: 0 8px 20px rgba(2, 132, 199, 0.35) !important;"
...
const old_button_2 = `<a :href="packageWaLink(packages[1])" target="_blank" rel="noopener noreferrer"
...
const new_button_2 = `<button type="button" @click="openPackageModal('intensif')"
                                style="background: linear-gradient(135deg, #fbbf24 0%, #f59e0b 50%, #d97706 100%) !important; color: #0b1727 !important; box-shadow: 0 8px 24px rgba(245, 158, 11, 0.4) !important;"
                                class="w-full py-4 px-6 rounded-2xl font-black text-base transition-all duration-200 flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-95 cursor-pointer shadow-lg">
                                <span class="icon-[mdi--pencil-plus] text-xl"></span>
                                Pilih Paket Intensif
...
const old_button_3 = `<a :href="packageWaLink(packages[2])" target="_blank" rel="noopener noreferrer"
...
const new_button_3 = `<button type="button" @click="openPackageModal('internasional')"
                                style="background: linear-gradient(135deg, #7c3aed 0%, #6d28d9 100%) !important; color: #ffffff !important; box-shadow: 0 8px 20px rgba(124, 58, 237, 0.35) !important;"
                                class="w-full py-4 px-6 rounded-2xl font-black text-base transition-all duration-200 flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-95 cursor-pointer shadow-lg">
                                <span class="icon-[mdi--pencil-plus] text-xl"></span>
                                Pilih Paket Internasional
...
const new_modal_and_script = `    <!-- ===== POP-UP MODAL FORM KEBUTUHAN PRIVAT SESUAI PAKET ===== -->
...
const fallback_pattern = /<div x-data>\s*<!-- Floating Action Button -->[\s\S]*?<!-- Event Calendar Modal -->/;
if (fallback_pattern.test(content)) {
    content = content.replace(fallback_pattern, new_modal_and_script + '\n    <!-- Event Calendar Modal -->');
    console.log("SUCCESS: Modal replaced successfully!");
} else {
    console.log("ERROR: Fallback pattern not found in content!");
}

// Inject privatApp script definition before </body>
const script_to_inject = `    <!-- ===== PRIVAT APP STATE MACHINE & DYNAMIC PACKAGE LOGIC ===== -->
    <script>
        function privatApp() {
            return {
...
fs.writeFileSync(privatPath, content, 'utf8');
console.log('SUCCESS: privat/index.html updated with Node.js script!');
