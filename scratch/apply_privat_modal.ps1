...
$content = $content.Replace(
    '<body class="min-h-screen bg-surface font-sans selection:bg-primary-fixed selection:text-on-primary-fixed overflow-x-hidden flex flex-col text-on-surface">',
...
$old_btn_1 = @'
                            <a :href="packageWaLink(packages[0])" target="_blank" rel="noopener noreferrer"
...
$old_btn_2 = @'
                            <a :href="packageWaLink(packages[1])" target="_blank" rel="noopener noreferrer"
...
$new_btn_2 = @'
                            <button type="button" @click="openPackageModal('intensif')"
                                style="background: linear-gradient(135deg, #fbbf24 0%, #f59e0b 50%, #d97706 100%) !important; color: #0b1727 !important; box-shadow: 0 8px 24px rgba(245, 158, 11, 0.4) !important;"
                                class="w-full py-4 px-6 rounded-2xl font-black text-base transition-all duration-200 flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-95 cursor-pointer shadow-lg">
                                <span class="icon-[mdi--pencil-plus] text-xl"></span>
                                Pilih Paket Intensif
...
$old_btn_3 = @'
                            <a :href="packageWaLink(packages[2])" target="_blank" rel="noopener noreferrer"
...
$new_btn_3 = @'
                            <button type="button" @click="openPackageModal('internasional')"
                                style="background: linear-gradient(135deg, #7c3aed 0%, #6d28d9 100%) !important; color: #ffffff !important; box-shadow: 0 8px 20px rgba(124, 58, 237, 0.35) !important;"
                                class="w-full py-4 px-6 rounded-2xl font-black text-base transition-all duration-200 flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-95 cursor-pointer shadow-lg">
                                <span class="icon-[mdi--pencil-plus] text-xl"></span>
                                Pilih Paket Internasional
...
# 3. New pop-up modal and floating button
$new_modal = @'
...
$oldModalStart = '<div x-data>'
$oldModalEnd = '<!-- Event Calendar Modal -->'

$pattern = '(?s)<div x-data>\s*<!-- Floating Action Button -->.*?<!-- Event Calendar Modal -->'
$content = [System.Text.RegularExpressions.Regex]::Replace($content, $pattern, $new_modal + "`r`n    <!-- Event Calendar Modal -->")

# 4. Inject privatApp script definition before </body>
$script_inject = @'
    <!-- ===== PRIVAT APP STATE MACHINE & DYNAMIC PACKAGE LOGIC ===== -->
    <script>
        function privatApp() {
            return {
                ...
                submitForm() {
                    ...
                    const waUrl = 'https://wa.me/' + adminWa + '?text=' + [System.Uri]::EscapeDataString($text);
...
$content = $content.Replace('</body>', $script_inject + "`r`n</body>")

[System.IO.File]::WriteAllText($filePath, $content, [System.Text.Encoding]::UTF8)
Write-Host "SUCCESS: PowerShell script updated privat/index.html!"
