...
# Replace the modal block in content
$pattern = '(?s)<!-- ===== POP-UP MODAL FORM KEBUTUHAN PRIVAT SESUAI PAKET ===== -->.*?<!-- Event Calendar Modal -->'
$content = [System.Text.RegularExpressions.Regex]::Replace($content, $pattern, $newModal + "`r`n    <!-- Event Calendar Modal -->")

# Remove redundant privatApp script at bottom
$scriptPattern = '(?s)<!-- ===== PRIVAT APP STATE MACHINE & DYNAMIC PACKAGE LOGIC ===== -->.*?<\/script>'
$content = [System.Text.RegularExpressions.Regex]::Replace($content, $scriptPattern, "")
