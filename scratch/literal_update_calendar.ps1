# In PowerShell, @' ... '@ is 100% literal string (no variable expansion)
$calendarJsBlock = @'
                // Filtered Events
                filteredEvents() {
...
$targetPattern = '(?s)(// Filtered Events|filteredEvents\(\)\s*\{).*?getEventCategoryBadge\(cat\)\s*\{.*?default:.*?\}.*?\},'

foreach ($fPath in $files) {
    $content = [System.IO.File]::ReadAllText($fPath, [System.Text.Encoding]::UTF8)
    if ($content -match $targetPattern) {
        $content = [regex]::Replace($content, $targetPattern, $calendarJsBlock.Trim())
        [System.IO.File]::WriteAllText($fPath, $content, [System.Text.Encoding]::UTF8)
        Write-Host "Updated calendar block in $fPath"
    } else {
        Write-Host "Pattern NOT matched in $fPath"
    }
}

# Update formatEventDateRange in nlsadmin
...
$adminRangePattern = '(?s)formatEventDateRange\(startDate,\s*endDate\)\s*\{.*?return startDate.*?\;\s*\}\s*\},'
if ($adminContent -match $adminRangePattern) {
    $adminContent = [regex]::Replace($adminContent, $adminRangePattern, $adminRangeBlock.Trim())
    [System.IO.File]::WriteAllText($adminPath, $adminContent, [System.Text.Encoding]::UTF8)
    Write-Host "Updated formatEventDateRange in nlsadmin"
} else {
    $adminRangePattern2 = '(?s)formatEventDateRange\(startDate,\s*endDate\)\s*\{.*?\n\s*\},'
    if ($adminContent -match $adminRangePattern2) {
        $adminContent = [regex]::Replace($adminContent, $adminRangePattern2, $adminRangeBlock.Trim())
        [System.IO.File]::WriteAllText($adminPath, $adminContent, [System.Text.Encoding]::UTF8)
        Write-Host "Updated formatEventDateRange (pattern 2) in nlsadmin"
    }
}
