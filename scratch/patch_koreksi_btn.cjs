...
// 1. Add computed property to check if a result needs grading
const helperCode = `
                getOriginalNode(r) {
                    if (!r) return null;
                    const course = this.coursesData[r.courseId];
                    if (!course || !course.content) return null;
                    let foundNode = null;
                    course.content.forEach(ch => {
                        if (ch.nodes) {
                            ch.nodes.forEach(node => {
                                if (node.title === r.moduleTitle || node.id === r.moduleTitle) {
                                    foundNode = node;
                                }
                            });
                        }
                ...
                needsGrading(r) {
                    ...
                    if (!node || !node.questions) return false;
                    return node.questions.some(q => q.type === 'essai');
                },
`;
content = content.replace('get averageScore() {', helperCode + '                get averageScore() {');

// 2. Modify the "Detail" button in the table
const buttonAnchor = `<button @click="viewDetails(r)" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-lg transition-colors font-bold text-xs inline-flex items-center gap-1 opacity-0 group-hover:opacity-100 focus:opacity-100 border border-indigo-100">`;
const newButton = `
...
`;
content = content.replace(buttonAnchor + '\n                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>\n                                        Detail\n                                    </button>', newButton);

fs.writeFileSync('nlsadmin/lms-results.html', content, 'utf8');
console.log('Patched Koreksi Button');
