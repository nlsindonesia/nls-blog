...
const payloadString = JSON.stringify(masterCloudStore);
console.log('Optimized Master Payload Size:', payloadString.length, 'bytes');

httpsRequest(CLOUD_DB_URL, 'PUT', masterCloudStore)
    .then((res) => {
        console.log(`Cloud DB Response Status: ${res.status}`);
        if (res.status === 200) {
            console.log('✓ SUCCESS: All local datasets successfully migrated to Cloud JSON Storage!');
        } else {
            console.log('Response data:', res.data);
        }

        // Update api/cloud-db.js
        console.log('Updating fallback cache in api/cloud-db.js...');
        let cloudDbCode = fs.readFileSync('api/cloud-db.js', 'utf8');
        const cacheRegex = /let localMemoryCache\s*=\s*\{[\s\S]*?\n\};\n/;
        if (cacheRegex.test(cloudDbCode)) {
            const newCacheCode = `let localMemoryCache = ${JSON.stringify(masterCloudStore, null, 4)};\n`;
            cloudDbCode = cloudDbCode.replace(cacheRegex, newCacheCode);
            fs.writeFileSync('api/cloud-db.js', cloudDbCode, 'utf8');
            console.log('✓ api/cloud-db.js fallback cache updated successfully!');
        }

        console.log('\n====================================================');
        console.log('FINAL CLOUD MIGRATION SUMMARY:');
        console.log(`✓ Users: ${masterCloudStore.users.length} records`);
        console.log(`✓ Events: ${masterCloudStore.events.length} records (Chronologically sorted)`);
        console.log(`✓ Articles: ${masterCloudStore.articles.length} records (Newest first)`);
        console.log(`✓ Teachers: ${masterCloudStore.teachers.length} records (Rank & order sorted)`);
        console.log(`✓ Teacher Applications: ${masterCloudStore.teacherApplications.length} records`);
        console.log(`✓ Courses / Modules: ${masterCloudStore.courses.length} records (Categorized)`);
        console.log(`✓ Quiz Submissions: ${masterCloudStore.quizSubmissions.length} records`);
        console.log('====================================================');
        process.exit(0);
    })
    .catch((err) => {
