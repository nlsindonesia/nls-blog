...
# Complete, clean, robust JS block for kalenderApp() / superAdminApp()
CALENDAR_JS_METHODS = """
                // Filtered Events
                filteredEvents() {
...
for fpath in FILES:
    ...
    # Replace from filteredEvents() to getEventCategoryBadge block
    pattern = re.compile(r'// Filtered Events.*?getEventCategoryBadge\(cat\)\s*\{.*?default:.*?\}.*?\},', re.DOTALL)
    if pattern.search(content):
        content = pattern.sub(CALENDAR_JS_METHODS.strip(), content)
        print(f"Successfully replaced calendar methods in {fpath}")
    else:
        # Try alternate pattern
        pattern2 = re.compile(r'filteredEvents\(\)\s*\{.*?getEventCategoryBadge\(cat\)\s*\{.*?default:.*?\}.*?\},', re.DOTALL)
        if pattern2.search(content):
            content = pattern2.sub(CALENDAR_JS_METHODS.strip(), content)
            print(f"Successfully replaced (pattern2) in {fpath}")
        else:
            print(f"Warning: pattern not found in {fpath}")
            
    with open(fpath, 'w', encoding='utf-8') as f:
        f.write(content)

# Also ensure nlsadmin formatEventDateRange is clean
admin_path = r"c:\Users\vc\Documents\nls-blog-hame\nls-blog-hame\nlsadmin\index.html"
with open(admin_path, 'r', encoding='utf-8') as f:
    admin_content = f.read()

# Replace formatEventDateRange in nlsadmin
admin_format_range = """
                formatEventDateRange(startDate, endDate) {
                    if (!startDate) return '';
                    if (!endDate || endDate === startDate) {
...
admin_pattern = re.compile(r'formatEventDateRange\(startDate,\s*endDate\)\s*\{.*?return startDate \+ '' - '' \+ endDate;\s*\}\s*\},', re.DOTALL)
if admin_pattern.search(admin_content):
    admin_content = admin_pattern.sub(admin_format_range.strip(), admin_content)
    print("Replaced formatEventDateRange in nlsadmin.")
else:
    # Check if broken template string in nlsadmin
    admin_pattern2 = re.compile(r'formatEventDateRange\(startDate,\s*endDate\)\s*\{.*?\n\s*\},', re.DOTALL)
    if admin_pattern2.search(admin_content):
        admin_content = admin_pattern2.sub(admin_format_range.strip(), admin_content)
        print("Replaced formatEventDateRange (pattern2) in nlsadmin.")

with open(admin_path, 'w', encoding='utf-8') as f:
    f.write(admin_content)

print("ALL SCRIPTS UPDATED AND VERIFIED CLEAN!")
